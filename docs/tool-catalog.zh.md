...
| `@deepseek-ai/dsh-tool-snapshot` | `snapshot_create`、`snapshot_diff`、`snapshot_list`、`snapshot_restore` | `ctx.tools`、`ctx.snapshots`、`ctx.systemPrompt` | `tool/call`、`快照提供方经会话事件发出的 `snapshot/create` 与 `snapshot/restore`、`tool/result` | - | 基于工作区快照缝合的 snapshot_create/list/restore/diff。restore 由提供方以破坏性变更防护（审批缝合）把关；diff 卡片经由从规范值回放的 presentationMeta 渲染。 |
| `@deepseek-ai/dsh-tool-multiedit` | `multi_edit` | `ctx.tools`、`ctx.fs`、`ctx.snapshots`、`ctx.systemPrompt` | `tool/call`、`每个文件各一条 `fs/edit-intent`、`每次应用的编辑后的 `fs/observed`、`回滚点对应的 `snapshot/create` 与 `snapshot/restore` 会话事件、`tool/result` | - | 一次原子多文件字面量编辑：要么全部应用，要么工作区回滚到同一工具调用内捕获的编辑前快照（rollback: true 跳过审批门）。单文件语义复用单文件编辑工具；结果渲染为一张多文件 diff 卡片。 |
...
<a id="deepseek-aidsh-tool-snapshot"></a>

## `@deepseek-ai/dsh-tool-snapshot`

### `snapshot_create`

对工作区状态做快照，以便稍后回滚或对比。成本低：只捕获创建之后发生变化的文件。请在风险较高的多步变更前执行。
...
```json
{
  ...
  "properties": {
    "reason": {
      "type": "string",
      "description": "Short free-form reason recorded with the snapshot, e.g. \"before refactor of auth flow\"."
    }
  },
  "required": [
    "reason"
...
Source: [`packages/fs/tool-snapshot/src/index.ts`](../packages/fs/tool-snapshot/src/index.ts)

### `snapshot_diff`

快照与当前工作区之间的行级差异。显示快照跟踪路径中已修改、新增与删除的文件。
...
```json
{
  ...
  "properties": {
    "id": {
      "type": "string",
      "description": "Snapshot id from snapshot_create or snapshot_list."
    }
  },
  "required": [
    "id"
...
列出本会话的工作区快照，按时间从旧到新，包含各自的 id、原因与诚实的部分捕获标志。
...
将工作区文件恢复到某快照状态：重写有差异的文件，删除快照之后创建的文件。破坏性操作——当会丢弃更改时请求审批。未托管的路径（捕获失败或超限）会报告，而不是恢复。
...
基于工作区快照缝合的 snapshot_create/list/restore/diff。restore 由提供方以破坏性变更防护（审批缝合）把关；diff 卡片经由从规范值回放的 presentationMeta 渲染。


<a id="deepseek-aidsh-tool-multiedit"></a>

## `@deepseek-ai/dsh-tool-multiedit`

### `multi_edit`

跨文件原子地应用多个字面量文本编辑：要么全部应用，要么失败时把整个调用回滚到编辑前状态。每条编辑在一份文件里把字面量 old_string 替换为 new_string（与单文件编辑工具语义相同）。
    ...
    "edits": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "replace_all": {
            "type": "boolean",
            "description": "Replace all matches in this file. Defaults to false; when false, old_string must appear exactly once in that file."
          }
        },
        "required": [
          "file_path",
...
Source: [`packages/fs/tool-multiedit/src/index.ts`](../packages/fs/tool-multiedit/src/index.ts)

一次原子多文件字面量编辑：要么全部应用，要么工作区回滚到同一工具调用内捕获的编辑前快照（rollback: true 跳过审批门）。单文件语义复用单文件编辑工具；结果渲染为一张多文件 diff 卡片。
