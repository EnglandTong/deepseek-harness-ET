...
flowchart LR
  ...
  svc_snapshots["ctx.snapshots<br/>Workspace-snapshot provider seam"]
  pkg_snapshot_local["snapshot-local"]
  pkg_tool_snapshot["tool-snapshot"]
  pkg_tool_multiedit["tool-multiedit"]
  ...
  pkg_snapshot --> svc_snapshots
  pkg_snapshot_local --> svc_snapshots
  ...
  svc_snapshots --> pkg_tool_multiedit
  svc_snapshots --> pkg_tool_snapshot
...
| `ctx.fs` | `seam` | [`fs`](../packages/fs/fs) | [`fs-local`](../packages/fs/fs-local), [`fs-sandbox`](../packages/fs/fs-sandbox), [`fs-e2b`](../packages/e2b/fs-e2b) | [`tool-fs`](../packages/fs/tool-fs) | [`fs-observation-policy`](../packages/fs/fs-observation-policy) |
| `ctx.snapshots` | `seam` | [`snapshot`](../packages/fs/snapshot) | [`snapshot-local`](../packages/fs/snapshot-local) | [`tool-snapshot`](../packages/fs/tool-snapshot)、[`tool-multiedit`](../packages/fs/tool-multiedit) | - | snapshot-local 经 fs 意图瀑布懒捕获变更前内容到内容寻址 blob；tool-snapshot 投影 create/list/restore/diff，tool-multiedit 提供原子多文件编辑，失败时恢复其自有的回滚点快照（restore 重入 fs 写路径，因此沙箱与观察政策与会话工具写入一样参与）。 | tool-fs 通过 ctx.fs 执行读取／写入／编辑；fs-sandbox 按共享沙箱模式限制变更；fs-observation-policy 通过 fs/* 事件门禁贡献基于观测状态的检查。 |
