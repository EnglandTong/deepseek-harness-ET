...
[English](ACTIVE_PACKET.md) | 中文

## Desired Outcome

DeepSeek Harness 提供可选的原生 Personal Supervisor Cordis 插件。插件维护唯一主助理会话、跨项目任务总账和有界自动调度；用户只需在主会话中查询、确认、返修和验收，项目宿主绑定的 subagent 负责实际开发。

## User And Situation

用户同时推进多个项目和模型会话，需要自行记忆各会话的上下文、状态和下一步。Personal Supervisor 持久化这些事实并按需投影，避免把全部历史塞入不断增长的提示词。

## Current Stage Outcome

WO-00 和 WO-01 已通过独立 Stage Review。WO-02、WO-03、WO-05、WO-06 已实现并完成审查；WO-04 至 WO-13 已有聚焦实现和测试，但真实重启复审、发布 Bundle/UI 集成和最终 Independent QA 仍未完成。
...
- 第一版不提供关闭后的常驻执行、跨主机同步或多进程共同写入。
- 不自动纳管候选项目，不在策略文件中存储凭证，不允许同一项目多个写者。
- 不依赖 experimental Agent Team 包，不完整实现非软件领域执行工具。

## Acceptance Criteria

- [Runtime] 首次启用只创建一个主助理会话，重启恢复同一身份。
- [Runtime] 两个项目并行执行时使用各自真实 `cwd`，同一项目只有一个写者。
- [Runtime] 主助理按策略调度模型或 CLI subagent，并合并需要确认的事项。
...
- [Governance] Developer、Stage Reviewer 和 Independent QA 权限分离，执行报告不能自动成为 Accepted。
...
- 后续阶段按 `.agents/state/personal-supervisor/WORK_ORDERS.md` 的依赖顺序修改授权 package、测试、示例和生成产物；每张 Work Order 由一个主要写者在独立分支完成。
...
- 不自动创建 worktree、部署、系统服务、force push 或破坏性 Git 操作。

## Evidence Required

- 每张工单的 focused tests、功能证据、diff 和原始结果。
...
- 主助理是自动创建并持久恢复的唯一 Session；其他对话的 `@总控` 只转交该 Session。
- 中央事件日志拥有项目注册和调度事实；项目 Packet/Loop Runs 拥有目标和执行证据；child Session 拥有完整对话。
...
运行 WO-14 集成门禁，覆盖组装后的 Bundle、Host API、客户端 Dashboard、持久化后端和无密钥快照；随后派发 WO-15 Independent QA。
