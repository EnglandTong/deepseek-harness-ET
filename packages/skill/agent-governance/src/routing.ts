import type { HarnessDescriptor, HarnessId, RouteRecommendation } from './types.ts'
...
export function recommend(task: string, agents: readonly HarnessDescriptor[]): RouteRecommendation {
  ...
  const primary = available.has(selected) ? selected : 'deepseek-harness'
  const alternatives = agents.filter(agent => agent.available && agent.id !== primary).map(agent => agent.id)
  return {
    task,
    primary,
    alternatives,
    reasons: [DEFAULT_RULES.find(rule => rule.primary === primary)?.reason ?? 'general task decomposition'],
