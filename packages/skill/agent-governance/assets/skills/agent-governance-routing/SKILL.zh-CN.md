...
先使用 `governance_list_agents` 检查可用 Agent，再使用 `governance_route_task` 生成推荐。路由推荐不是执行授权。
...
除非任务明确指定第二个项目且已经获得人工批准，否则禁止跨项目目录委派。将任务包保存在 `.agent-state/<task-id>/`，通过文件引用交接，不要复制完整对话历史。
