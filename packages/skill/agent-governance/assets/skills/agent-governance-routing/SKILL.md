...
Use `governance_list_agents` before selecting an external Agent. Use `governance_route_task` to create a recommendation; routing is not execution authority.
...
Do not delegate across project directories unless the task explicitly names the second project and a human has approved it. Keep the task packet in `.agent-state/<task-id>/` and pass references instead of copying the full conversation.
