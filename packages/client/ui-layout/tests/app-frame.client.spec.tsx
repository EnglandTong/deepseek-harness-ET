 ...
 * store instance (createLayoutStore().create() 鈥?the test-sanctioned engine
...
// mode runs the empty branch 鈥?the frame must work against exactly this
...
describe('AppFrame', () => {
  ...
  it('keeps the conversation slot mounted while no session is current', () => {
    ...
    // Session view itself 鈥?the center column renders it unconditionally.
  ...
  it('renders both column occupants before baselines settle (no loading gate)', () => {
    ...
    // pending rendering 鈥?both occupants mount from first paint.
  ...
  it('the aside column stays mounted at zero width and toggles from the store', () => {
    const { frame, getAllByTestId, instance } = mountFrame()
    expect(getAllByTestId('other-content').length).toBeGreaterThanOrEqual(1)
    expect(frame.hasAttribute('data-aside-collapsed')).toBe(true)
    act(() => { instance.actions.toggleAside() })
    expect(tracks(frame)[2]).toBe(360)
    expect(frame.hasAttribute('data-aside-collapsed')).toBe(false)
    act(() => { instance.actions.toggleAside() })
...
describe('AppFrame 鈥?narrow-viewport auto-collapse', () => {
...
describe('AppFrame 鈥?guard branches', () => {
...
describe('AppFrame 鈥?unmount with an in-flight resize frame', () => {
