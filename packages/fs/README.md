...
| `snapshot/` | Service Definition: workspace-snapshot identity, create/list/restore/diff contract, and the `snapshot/create` + `snapshot/restore` session events | `ctx.snapshots` |
| `snapshot-local/` | Local snapshot provider: lazy pre-change capture through the `fs/write-intent`/`fs/edit-intent` waterfalls into content-addressed blobs; restore re-enters the fs write path (sandbox + observation policy participate) and gates destructive restores on approval | (registers `ctx.snapshots`) |
| `tool-snapshot/` | Model-facing `snapshot_create`/`snapshot_list`/`snapshot_restore`/`snapshot_diff` tools; restore/diff render as multi-file diff cards | (registers on `ctx.tools`) |
| `tool-multiedit/` | Model-facing atomic `multi_edit` tool: multi-file literal edits whose failure restores the call's own rollback-point snapshot (`rollback: true`) | (registers on `ctx.tools`) |
