...
- **Startup and discovery timeouts bound the sync, not the SDK default** — `startupTimeoutMs` (default 60000) bounds the initial `initialize` handshake and each paginated `tools/list` discovery request; an unresponsive server or cursor chain delays activation and teardown up to that bound while the initial synchronization settles.
